"""
与原版不同的部分在于加入了图像的部分空间信息，本版本的伪标签生成模块，不仅仅随机组合超像素进行预测。
而是在预测某个超像素类别时，与多个基准类别超像素进行组合，并根据超像素之间的空间距离赋予多个得分权重。
最后加权求和得到该待预测超像素的置信度。
在再检查的流程中，不再随机选取整张图像的基准类别超像素进行拼接预测，而是随机选取空间距离较近的基准类别超像素。
"""
...
class WESUP(nn.Module):
    ...
    def _pred_relation_forward(self, sp_labels, sp_features,
        ...
        def get_relation_forward(sp_labels, sp_features):
            base_class = 1
            base_class_index = torch.where(sp_labels[:, 1] == base_class)[
                0].cpu().numpy()
            ...
            # train_relation_weight = min(64., sp_num)
        ...
        relation_pred, relation_target = get_relation_forward(
            sp_labels, sp_features)

        if pseudo_train_labels is not None:
            pseudo_train_labels = torch.cat(
                (pseudo_train_labels, sp_labels), 0)
            pseudo_train_features = torch.cat(
                (pseudo_train_features, sp_features), 0)

            pseudo_relation_pred, pseudo_relation_target = get_relation_forward(
                pseudo_train_labels, pseudo_train_features)
        else:
    ...
    def _get_pseudo_lables(self, sp_maps, sp_labels, sp_features, p_features):
        ...
        negative_index = torch.where(sp_labels[:, 1] == 0)[0].cpu().numpy()
        positive_index = torch.where(sp_labels[:, 1] == 1)[0].cpu().numpy()
        ...
        sp_coordinate = torch.zeros(sp_num, 2).to(device).detach()
        # GT
        for sp_idx in range(n_l):
            pseudo_labels[sp_maps == sp_idx] = sp_labels[sp_idx][1]

        for sp_idx in range(sp_num):
            sp_mask = sp_maps == sp_idx
            sp_index = torch.nonzero(sp_mask)
            for i in range(2):
                sp_coordinate[sp_idx][i] = torch.div(
                    sp_index[:, i].max() + sp_index[:, i].min(), 2, rounding_mode='trunc')

        base_class_sp_coordinate = sp_coordinate[base_class_index]

        pseudo_positive_index = []
        pseudo_negative_index = []

        recheck_sp_index = []
        confused_sp_index = []
        confused_sp_confidence = []

        pseudo_labels_for_relation_train = []
        pseudo_features_for_relation_train = []
        maxx_distance = np.sqrt(height ** 2 + width ** 2)

        for sp_idx in range(n_l, sp_num):
            sp_distance = (
                base_class_sp_coordinate[:, 0] - sp_coordinate[sp_idx][0]).pow(2)
            sp_distance = sp_distance + \
                (base_class_sp_coordinate[:, 1] -
                 sp_coordinate[sp_idx][1]).pow(2)
            sp_distance = sp_distance.sqrt()
            sp_distance = (maxx_distance - sp_distance) / maxx_distance
            sp_distance = sp_distance.exp()
            sp_distance = F.softmax(sp_distance, dim=0)

            select_sp_feature = sp_features[sp_idx]
            inp = select_sp_feature.repeat(base_class_num, 1)
            inp = torch.cat((base_class_featrues, inp), 1)
            pred = self.relation_classifier(inp)[:, 1]
            pred = (pred * sp_distance).sum()

            if pred > top1:
                pseudo_labels[sp_maps == sp_idx] = positive_pseudo_tag
                pseudo_positive_index.append(sp_idx)

                pseudo_features_for_relation_train.append(select_sp_feature)
                pseudo_labels_for_relation_train.append(torch.tensor((0, 1)))
            ...
            if pred > top2:
                ...
                confused_sp_index.append(sp_idx)
                confused_sp_confidence.append(pred)
                recheck_sp_index.append(sp_idx)

            elif pred < bottom1:
                pseudo_labels[sp_maps == sp_idx] = negative_pseudo_tag
                pseudo_negative_index.append(sp_idx)

                pseudo_features_for_relation_train.append(select_sp_feature)
                pseudo_labels_for_relation_train.append(torch.tensor((1, 0)))
        ...
        if base_class == 0:
            base_class_pseudo_sp_coordinate = torch.cat(
                (base_class_sp_coordinate, sp_coordinate[pseudo_negative_index]), 0)

            base_class_pseudo_index = np.concatenate(
                [base_class_index, np.array(pseudo_negative_index)], axis=0)
            base_class_pseudo_index = torch.Tensor(
                base_class_pseudo_index).to(device)
        elif base_class == 1:
            base_class_pseudo_sp_coordinate = torch.cat(
                (base_class_sp_coordinate, sp_coordinate[pseudo_positive_index]), 0)

            base_class_pseudo_index = np.concatenate(
                [base_class_index, np.array(pseudo_positive_index)], axis=0)
            base_class_pseudo_index = torch.Tensor(
                base_class_pseudo_index).to(device)
        else:
            assert('class error')

        labeled_num = len(positive_index) + len(negative_index) + \
            len(pseudo_positive_index) + len(pseudo_negative_index)
        enable_p_prediction = labeled_num / sp_num > 0.8

        for sp_idx in recheck_sp_index:
            sp_distance = (
                base_class_pseudo_sp_coordinate[:, 0] - sp_coordinate[sp_idx][0]).pow(2)
            sp_distance = sp_distance + \
                (base_class_pseudo_sp_coordinate[:,
                 1] - sp_coordinate[sp_idx][1]).pow(2)

            sp_mask = (sp_maps == sp_idx).view(p_features.size(0))
            sp_mask_index = torch.nonzero(sp_mask)[:, 0].cpu().numpy()

            _, indices = torch.sort(sp_distance)
            random_range = min(10, indices.size(0))
            distance_pick_index = indices[np.random.randint(random_range)]
            pick_index = base_class_pseudo_index[distance_pick_index].int(
            ).item()
            first_features = sp_features[pick_index]

            if enable_p_prediction:
                # p_wish
                need_judge_features = p_features[sp_mask_index]
                inp = first_features.repeat(len(sp_mask_index), 1)
                inp = torch.cat((inp, need_judge_features), 1)
                pixel_pred = self.relation_classifier(inp)[:, 1]

                for it, pixel_index in enumerate(sp_mask_index):
                    x = pixel_index // width
                    y = pixel_index % width

                    if pixel_pred[it] > top1:
                        pseudo_labels[x][y] = positive_pseudo_tag
                        pseudo_features_for_relation_train.append(
                            need_judge_features[it])
                        pseudo_labels_for_relation_train.append(
                            torch.tensor((0, 1)))

                    elif pixel_pred[it] > top2:
                        pseudo_labels[x][y] = positive_pseudo_tag

                    elif pixel_pred[it] < bottom1:
                        pseudo_labels[x][y] = negative_pseudo_tag
                        pseudo_features_for_relation_train.append(
                            need_judge_features[it])
                        pseudo_labels_for_relation_train.append(
                            torch.tensor((1, 0)))

                    elif pixel_pred[it] < bottom2:
                        pseudo_labels[x][y] = negative_pseudo_tag

                    else:
                        if pixel_pred[it] > 0.5:
                            pseudo_labels[x][y] = -1
                        else:
                            pseudo_labels[x][y] = -2
            else:
                ...
                select_sp_feature = sp_features[sp_idx]
                inp = torch.cat(
                    (first_features, select_sp_feature), 0).unsqueeze(0)
                pred = self.relation_classifier(inp)[:, 1]
                ...
                if pred > top1:
                    ...
                    if pred > 0.5:
                        pseudo_labels[sp_maps == sp_idx] = -1
                    else:
                        pseudo_labels[sp_maps == sp_idx] = -2

        pseudo_labels = pseudo_labels.unsqueeze(0)

        if len(pseudo_labels_for_relation_train) > 0:
            pseudo_train_num = max(
                len(pseudo_labels_for_relation_train), n_l * 8)

            if pseudo_train_num == len(pseudo_labels_for_relation_train):
                pseudo_train_index = np.random.choice(
                    range(len(pseudo_labels_for_relation_train)), size=pseudo_train_num)
            else:
                pseudo_train_index = np.random.permutation(
                    len(pseudo_labels_for_relation_train))

            pseudo_labels_for_relation_train = [
                pseudo_labels_for_relation_train[i] for i in pseudo_train_index]
            pseudo_features_for_relation_train = [
                pseudo_features_for_relation_train[i] for i in pseudo_train_index]

            pseudo_train_labels = torch.stack(
                pseudo_labels_for_relation_train).to(device)
            pseudo_train_features = torch.stack(
                pseudo_features_for_relation_train).to(device)
        else:
